...
Scans a video for flashing content that fails **WCAG 2.3.1**, suggests the edit that would fix each event, and exports a report — entirely in the browser. Nothing is uploaded.
...
- **Fixes:** each event carries suggested edit operations (dim the range, slow it to 3 flashes/s, desaturate red) as JSON payloads. The checker does not apply them; the editor would, through its existing colour and speed controls, so fixes share the normal undo stack and renderer.
- **Report:** "Download report" saves a JSON file with the verdict, every event, and its suggested fixes.
